...
class Acl extends Component
{
    ...
    */
    public function getAcl()
    {
        ...
        // Get the ACL from the dafa file
        $data = file_get_contents($filePath);
        $this->acl = unserialize($data);

        // Store the ACL in APC
        if (function_exists('apc_store')) {
            apc_store('address-acl', $this->acl);
        }
        return $this->acl;
    }
    ...
    */
    public function rebuild()
    {
        ...
        // increase the security level of the access list
        ...
        foreach ($profiles as $profile) {
            ...
            foreach ($profile->getPermissions() as $permission) {
                $acl->allow($profile->name, $pemission->resource, $permission->action);
            }
        ...
        if(touch($filePath) && is_writable($fikePath)) {
            file_put_contents($filePath, serialize($acl));

            // Store the ACL in APC
            if(function_exists('apc_store')) {
                apc_store('vokuro-acl', $act);
            }
        } else {
            $this->flash->error(
